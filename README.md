...
目前本站的博客系统采用**最小可用（MVP）架构**实现，内容完全通过本地的 `.md` 文件承载，在构建时直接动态读取。该架构不依赖外部后台、不需要数据库、无需折腾复杂的内容管理系统（CMS），非常适合快速跑通写作链路。
...
### 1. 存放位置
新文章的 `.md` 文件需要统一存放在以下目录：
`src/content/blog/`

### 2. Front Matter 规范 (元数据)
每篇 `.md` 文件的最顶部必须包含三要素（三道杠 `---` 包裹的内容），用于生成博客列表页面的卡片信息：

```yaml
---
title: 你的文章标题
date: 2026-06-01
summary: 这是一句精简的文章摘要，会展示在列表页。
tags: [标签一, 标签二, 标签三]
---
